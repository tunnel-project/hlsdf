    ...
    if response.status_code == 200:
        try:
            ...
            for proxy in proxies:
                if proxy['type'] == 'ss':
                    ss_url = f"ss://{base64.b64encode(f'{proxy['cipher']}:{proxy['password']}'.encode()).decode()}@{proxy['server']}:{proxy['port']}#{urllib.parse.quote(proxy['name'])}"
    ...
    for i in range(0, len(configs), 6):
        ...
        try:
            ...
            if message_count % 10 == 0:
                await send_config_file(bot, all_configs[-50:])

            await asyncio.sleep(2)  # 2-second delay between messages
        except Exception as e:
            logger.error(f"Failed to send configs to Telegram: {str(e)}")

    # Send the final batch if it hasn't been sent yet
    if message_count % 10 != 0:
        await send_config_file(bot, all_configs[-50:])

    await bot.close()

async def send_config_file(bot: Bot, configs: List[str]):
    config_text = "\n".join(configs)
    file_obj = BytesIO(config_text.encode('utf-8'))
    file_obj.name = "latest_50_configs.txt"

    try:
        caption = f"محتوای فایل رو کپی و در برنامه مورد نظر کپی کنید.\n #فایل"
