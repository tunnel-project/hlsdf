...
def send_configs_to_telegram(configs: List[str]):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHANNEL_ID:
        logger.error("Telegram bot token or channel ID not set")
        return

    base_url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendDocument"
    
    content = "Latest Configs:\n\n"
    for config in configs:
        content += f"{config}\n\n"

    files = {
        'document': ('latest_configs.txt', content)
    }

    data = {
        'chat_id': int(TELEGRAM_CHANNEL_ID),
        'caption': f'Latest {len(configs)} Configs'
    }

    response = requests.post(base_url, data=data, files=files)
    if response.status_code != 200:
        logger.error(f"Failed to send configs to Telegram: {response.text}")
    else:
        logger.info(f"Successfully sent {len(configs)} configs to Telegram")
...
async def main():
    ...
    try:
        ...
        all_configs = modified_premium_configs + modified_configs
        latest_configs = all_configs[:300]  # Get only the latest 300 configs

        # Send configs to Telegram
        send_configs_to_telegram(latest_configs)
        
        logger.info(f"Successfully processed and sent {len(latest_configs)} configs")
