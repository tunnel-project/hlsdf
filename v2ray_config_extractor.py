...
async def send_configs_to_telegram(configs: List[str]):
    ...
    for i in range(0, len(configs), 6):
        ...
        try:
            ...
            await asyncio.sleep(30)  # If an error occurs, wait for 30 seconds before continuing
...
منتظر نظرات و پیشنهادات شما هستیم! ❤️""" 
    ...
    await asyncio.sleep(5)  # Add a delay before sending the final message
    await bot.send_message(chat_id=TELEGRAM_CHANNEL_ID, text=final_message)
...
async def send_config_file(bot: Bot, configs: List[str]):
    ...
    try:
        ...
        await asyncio.sleep(5)  # Add a delay after sending the file
    except Exception as e:
